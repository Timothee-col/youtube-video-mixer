...
# Avertissement anti-bot
with st.expander("🛡️ Aide - Problèmes YouTube", expanded=False):
    st.warning("**Si vous voyez 'Sign in to confirm you're not a bot':**")
    st.markdown("""
    **Solutions rapides :**
    1. **Changez l'URL** : Remplacez `youtube.com` par `m.youtube.com`
    2. **Essayez youtu.be** : Utilisez le format court `https://youtu.be/VIDEO_ID`
    3. **Vidéos courtes** : Les vidéos < 5min passent souvent mieux
    4. **Attendez** : Parfois ça marche après quelques minutes
        ...
        if downloaded_files:
            ...
            for idx, video_info in enumerate(downloaded_files):
                ...
                clips = extract_best_clips_with_face(
                    ...
                    face_threshold=face_threshold if 'face_threshold' in locals() else 0.4
                )
